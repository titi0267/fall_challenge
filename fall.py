...
def getParams(params: list):
    ...
    for i in range(height):
        ...
        for j in range(width):
            ...
            if own == 1:
                if i >= height/2:
                    highPriorityMovment.append(1)
                else:
                    highPriorityMovment.append(3)
                if j >= width/2:
                    highPriorityMovment.append(4)
                else:
                    highPriorityMovment.append(2)
...
def selectTilesForSpawn(tileLeft: Field or None, tileRight: Field or None, tileUp: Field or None, tileDown: Field or None, tileType: int):
    tileTypeArray = [tileType]
...
def prioritySpawn(tileLeft: Field or None, tileRight: Field or None, tileUp: Field or None, tileDown: Field or None, exclude: list, loopedNumber: int):
    ...
    myTiles = []
    freeTiles = []
    opponentTile = selectTilesForSpawn(tileLeft, tileRight, tileUp, tileDown, 0)
    myTiles = selectTilesForSpawn(tileLeft, tileRight, tileUp, tileDown, 1)
    freeTiles = selectTilesForSpawn(tileLeft, tileRight, tileUp, tileDown, -1)
                    ...
                    if len(moveTo) > 0 and type(moveTo[0]) == list: #Move toward enemy units
                        ...
                        for d in range(0, len(moveTo)):
                            if moveTo[d][0] <= unitsAmount:
                                if moveTo[d][0] == unitsAmount:
                                    ...
                                    movePrint(i, j, moveTo[d][1], moveTo[d][0] + 1)
                                unitsAmount -= moveTo[d][0] + 1
                        ...
                        if len(moveTo) - 1 > tiles[i][j].units: # Not Enough units to go in every given direction
                            ...
                            while len(possibleDirections) != 0 and unitsAmount > 0:
                                if moveTo[0] == -1:
                                    if turn % 2 == 0:
                                        if highPriorityMovment[0] in possibleDirections:
                                            movePrint(i, j, highPriorityMovment[0], 1)
                                            possibleDirections.remove(highPriorityMovment[0])
                                            unitsAmount -= 1
                                        elif highPriorityMovment[1] in possibleDirections:
                                            movePrint(i, j, highPriorityMovment[1], 1)
                                            possibleDirections.remove(highPriorityMovment[1])
                                            unitsAmount -= 1
                                        else:
                                        ...
                                        if highPriorityMovment[1] in possibleDirections:
                                            movePrint(i, j, highPriorityMovment[1], 1)
                                            possibleDirections.remove(highPriorityMovment[1])
                                            unitsAmount -= 1
                                        elif highPriorityMovment[0] in possibleDirections:
                                            movePrint(i, j, highPriorityMovment[0], 1)
                                            possibleDirections.remove(highPriorityMovment[0])
                                            unitsAmount -= 1
                                        else:
...
def spawn(params: tuple[list[list[Field]], list[int], tuple[int, int]]):
            ...
            for j in range(len(tiles[i])):
                if tiles[i][j].own == 1 and tiles[i][j].recycler == 0:
                    ...
                    if type(spawnHere) == list:
                        ...
                        if reinforcment > 0 and (tiles[i][j].in_range == 0 or (tiles[i][j].in_range == 1 and tiles[i][j].scrap > 1)):
    ...
    for g in range(len(tempMatter)):
        ...
        if tiles[tempMatter[g]['pos'][1]][tempMatter[g]['pos'][0]].units == 0:
            recyclerCost += 10
    #print("MESSAGE", str(tempMatter), str(d)+";", end="")
    print("MESSAGE", str(recyclerCost), str(tempMatter)+";", end="")
    for d in range(0, len(tempMatter)):
        if (recyclerCost + tempMatter[d]['cost'] > totalMatter): #Not or just enough matter to block opponent everywhere
            return (totalMatter, tempMatter, d)
        if (totalMatter < int(tempMatter[d]['cost'])): #Not enough matter to block more tiles
            return (totalMatter, tempMatter, d)
        if (recyclerCost + tempMatter[d]['cost'] + 10 < totalMatter and totalMatter > int(tempMatter[d]['cost']) + 10): #Not or just enough matter to block opponent everywhere
            print("SPAWN", str(int(math.floor(tempMatter[d]['cost']/10)) + 1), str(tempMatter[d]['pos'][0]), str(tempMatter[d]['pos'][1])+";", end="")
            totalMatter -= (tempMatter[d]['cost'] + 10)
        else:
            print("SPAWN", str(int(math.floor(tempMatter[d]['cost']/10))), str(tempMatter[d]['pos'][0]), str(tempMatter[d]['pos'][1])+";", end="")
...
def build(params: tuple[list[list[Field]], list[int], tuple[int, int]], matterNeeded: list[dict[str, int]]):
    ...
    for i in range(len(matterNeeded)):
        if params[1][0] < 10:
            return params
        if tiles[matterNeeded[i]['pos'][1]][matterNeeded[i]['pos'][0]].units == 0:
